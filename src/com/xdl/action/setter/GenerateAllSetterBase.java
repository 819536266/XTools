...
public abstract class GenerateAllSetterBase extends PsiElementBaseIntentionAction {
    ...
    private String generateStringForParam(String generateName,
        ...
        for (PsiMethod method : methodList) {
            String setterMethodNamePrefix =
                    method.getName()
                            .startsWith(SET_SETTER_PREFIX)
                            ? SET_SETTER_PREFIX
                            : method.getName()
                            .startsWith(WITH_SETTER_PREFIX)
                            ? WITH_SETTER_PREFIX
                            : null;
            if (setterMethodNamePrefix != null) {
                String fieldToLower = method.getName()
                        .substring(setterMethodNamePrefix.length())
                        .toLowerCase();
                PsiMethod s = info.getNameToMethodMap()
                        .get(fieldToLower);
                if (s != null) {
    ...
    private static String extractSplitText(PsiMethod method,
                                           Document document) {
        int startOffset = method.getTextRange()
                .getStartOffset();
    ...
    private void handleWithLocalVariable(PsiLocalVariable localVariable,
        ...
        document.insertString(parent1.getTextOffset() + parent1.getText()
                        .length(),
                buildString);
        PsiDocumentUtils.commitAndSaveDocument(psiDocumentManager, document);
        PsiToolUtils.addImportToFile(psiDocumentManager,
            ...
            if (ss != null) {
                ...
                if (paramInfo.getCollectName() != null && guavaTypeMaps
                    ...
                    if (paramInfo.getCollectName() != null) {
                        ...
                        String realPackage = paramInfo.getParams()
                                .get(0)
                                .getRealPackage();
                        ...
                        if (s != null) {
                            ...
                            Arrays.stream(allFields)
                                    .findFirst()
                                    .ifPresent(field -> builder.append(psiClassOfParameter.getName())
                                            .append(".")
    ...
    private static void appendCollectNotEmpty(StringBuilder builder,
        ...
        for (int i = 0; i < paramInfo.getParams()
                .size(); i++) {
            builder.append(paramInfo.getParams()
                    .get(i)
                    .getRealName());
            newImportList.add(paramInfo.getParams()
                    .get(i)
                    .getRealPackage());
            if (i != paramInfo.getParams()
                    .size() - 1) {
                builder.append(",");
            }
        }
    ...
    public boolean isAvailable(@NotNull Project project, Editor editor,
        ...
        if (generateAllHandler.forBuilder()) {
            ...
            for (PsiMethod method : methods) {
                if (method.getName()
                        .equals(CommonConstants.BUILDER_METHOD_NAME) && method.hasModifierProperty(STATIC)) {
                    return true;
                }
            }
            return false;
        }
        if (generateAllHandler.isAccessors()) {
            PsiClass localVarialbeContainingClass = getLocalVarialbeContainingClass(element);
            if (localVarialbeContainingClass == null) {
                return false;
            }
            PsiAnnotation annotation = localVarialbeContainingClass.getAnnotation("lombok.experimental.Accessors");
            if (annotation == null) {
                return false;
            }
            PsiAnnotationMemberValue chain = annotation.findAttributeValue("chain");
            if (chain == null || chain.getText() == null || !Boolean.parseBoolean(chain.getText())) {
                return false;
            }
            return true;
        }
