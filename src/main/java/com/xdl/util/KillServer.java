...
public class KillServer {
     ...
     * 执行关闭
     * @param text post
     * @param portContent   portContent
     */
    public void kill(String text, JTextArea portContent) {
        jTextArea = portContent;
        ...
        for (String spid : split) {
            try {
                ...
                println("错误的端口号，请输入一个或者多个端口，以英文逗号隔开");
            }
        }
        println("need kill " + ports.size() + " num");
//        System.out.println("need kill " + ports.size() + " num");
        for (Integer pid : ports) {
            start(pid);
        }
...
//                    System.out.println(string);
                ...
                println("找到" + read.size() + "个进程，正在准备清理");
